...
//}@dev
    ...
    if (ua) {
        ...
        _detectConnection(spec);  // NETWORK_CONNECTION

        spec["MOBILE"] = /Android|iOS|Windows Phone/.test(spec["OS_TYPE"]);
    }
    return spec;
}
...
//                                      //     SIMD(NEON) をサポートしている場合に true になります。Tegra 2 では false です。
        ...
        "NETWORK_CONNECTION": {         // [S] Device/Browser
...
function _detectConnection(spec) {
    var ver = parseFloat(spec["OS_VERSION"]);
    var cph = 6;
    var max = 16;
    ...
    spec["NETWORK_CONNECTION"] = { "MAX": max, "CPH": cph };
}
