...
Spec["prototype"] = Object.create(Spec, {
    ...
    "LANGUAGE":             { "get":   function()  { return this._LANGUAGE;         } }, // Content language. "en-us" -> "en"
    // --- DISPLAY ---
...
Spec["FEATURES"] = {
    "DETECTED":             0,
    "WEBP":                 0x0,    // WebP: 0x0 = unsupported, 0x1 = lossy, 0x2 = lossless, 0x4 = alpha, 0x8 = animation
    "CPU_MAX_THREADS":      2,      // CPU: navigator.hardwareConcurrency
    "CPU_SPEED_THRESHOLD":  1.2,    // CPU: speed threshold.
    "CPU_CORES_THRESHOLD":  2,      // CPU: cores threshold.
    "GPU_GFLOPS_THRESHOLD": 19,     // GPU: GFLOPS threshold.
    "GPU_MAX_TEXTURE_SIZE": 0,      // GPU: MAX_TEXTURE_SIZE (1024 - 16384)
...
function _init(ua) { // @arg String
    ...
    if (devid && osName) {
        ...
        if (devData) {
            ...
            this._SLOW_CPU          = socData[1] <= Spec["FEATURES"]["CPU_SPEED_THRESHOLD"] &&
                                      socData[2] <= Spec["FEATURES"]["CPU_CORES_THRESHOLD"];
            this._SLOW_GPU          = gpuData[0] <= Spec["FEATURES"]["GPU_GFLOPS_THRESHOLD"];
...
function _isOldDevice(yymm,         // @arg Number - device release date. yymm
    ...
    // 30 months elapsed -> old device
    if (this._CURRENT_TIME > releaseDate + 2.5 * year + month) {
        return true;
    }
    // 24 months and not version-up -> desert device.
    if (this._CURRENT_TIME > releaseDate + 2.0 * year + month) {
...
function _getiOSDeviceID(ua, osVer) {
  ...
  //var SGX535 = /535/.test(glVersion); // iPhone 3GS, iPhone 4
    ...
    var A8     = /A8/.test(glVersion);  // iPhone 6/6+, iPad Air 2
