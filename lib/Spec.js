...
function Spec() { // @desc http://git.io/Spec
    ...
    // --- OS ---
    this._OS_NAME           = "";       // OS name. eg: "Android", "iOS", "Windows", "Mac", "Firefox", "Fire", "Console"
    ...
    this._ENABLE_CHROME_TRIGGER = false;
...
Spec["prototype"] = Object.create(Spec, {
    ...
    "ENABLE_CHROME_TRIGGER":{ "get":   function()  { return this._ENABLE_CHROME_TRIGGER; } },
...
function Spec_setUserAgent(ua) { // @arg UserAgentString
    if (ua && this._USER_AGENT !== ua) {
        ...
        this._MAX_CONNECTIONS = (aosp && ver < 4) ? 10 : 16;
        this._CONNECTIONS_PER_HOST = aosp ? (ver < 4 ? 8 : 6)
                                   : browserName === "IE" ? (ver < 11 ? 8 : 13) : 6;
        this._ENABLE_CHROME_TRIGGER = (!this._WEB_VIEW && aosp && ver >= 4 && ver < 4.4);
...
function _collectDeviceInfo() {
    ...
    } else { // unknown device -> alternative device id
        switch (this._OS_NAME) {
        case "Android": deviceID = "Nexus 5"; break;
        case "iOS":     deviceID = "iPhone 5";
        }
        devInfo = Spec["DEVICE"]["CATALOG"][deviceID];
        this._DEVICE_DETECTED   = false;
        this._DEVICE_CANDIDATE  = [];
    }
...
function _detectDeviceID(ua, osName) {
    ...
    case "Windows": return /Windows Phone/.test(ua) ? _getWindowsPhoneDeviceID(ua)
                                                    : "";
