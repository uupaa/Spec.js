...
function Spec_hasApp(name) {
    return !!(this._APP & Catalog["APP"][name]);
}
function Spec_hasFeature(name) {
    var value = Catalog["FEATURE"][name];
    if (value) {
...
function _collectDeviceInfo(deviceID, // "Nexus 5"
                            osName) { // "Android"
    var devs = Catalog["DEVICE"][deviceID]; // [ SOC_ID, OS_VERS, DISP1, DISP2, INCH, RAM, FEATURE, LIMIT, APP ]
    ...
    if (devs) {
        ...
        this._FEATURE.DEVICE    = devs[6];
        this._FEATURE.SOC       = socs[3];
        this._FEATURE.GPU       = gpus[3];
        this._LIMIT.DEVICE      = devs[7];
        ...
        this._CPU_CORES         = nav["hardwareConcurrency"] || socs[1] || 1;
        this._GPU_ID            = gpuID;
...
//      this._GLES_VERSION      = this._GLES_VERSION  || gpus[1] || 0.0;
        ...
        this._MAX_TOUCH_POINTS  = nav["maxTouchPoints"] ||
                                  _getMaxTouchPoints(this._LIMIT.DEVICE, Catalog["LIMIT"]);
    }
