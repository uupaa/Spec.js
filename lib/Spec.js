...
function Spec(env) { // @arg Object = {} - { dpr, touch, width, height, language, userAgent }
                     // @env.alt Boolean = false
                     // @env.dpr Number = 1.0
                     // @env.touch Integer = 0
                     // @env.width Integer = 0
                     // @env.height Integer = 0
                     // @env.language String = "en"
...
//}@dev
    ...
    // --- DEVICE ---
    this["ID"] = "";                // Device ID picked up from the UserAgent string. eg: "iPhone 5"
    this["SOC"] = "";               // System on chip name (UPPER_CASE STRING). eg: "MSM8974", "A5X"
    this["BRAND"] = "";             // Device brand name or device maker name (UPPER_CASE STRING). eg: "OTHER", "APPLE", "GOOGLE", "AMAZON"...
    this["FEATURE"] = 0;            // A bit pattern that indicates the features and functionality of the device.
    this["LANGUAGE"] = lang;        // Browser/Content language. "en", "ja", ...
    this["DETECTED"] = false;       // Device is described in the catalog.
    this["ID_CANDIDATE"] = [];      // Candidate the device ID lists. If cannot be device id determined. ["iPhone 5", "iPhone 5c", "iPhone 5s"]
    // --- OS ---
    this["OS"] = "";                // The OS name. eg: "Android", "iOS", "Windows Phone", "Windows", "Mac", "Firefox OS", "Fire OS", "Game"
    this["OS_VERSION"] = _000;      // Detected Current OS version from User Agent string. format "{{Major}},{{Minor}},{{Patch}}"
                                    // Convert to a floating point number are `parseFloat(spec["OS_VERSION"])`
    this["OS_RELEASE_VERSION"]=_000;// The OS version of the device at the release date. format "{{Major}},{{Minor}},{{Patch}}"
    this["OS_HIGHEST_VERSION"]=_000;// The OS version of the end of support. format "{{Major}},{{Minor}},{{Patch}}"
    // --- MEMORY / CPU / GPU ---
    ...
    this["BROWSER_NAME"] = "";      // Browser name. "Chrome", "Firefox", "Browser", "IE", "Safari", "WebKit"
                                    ...
                                    // Fire Phone      -> "Browser"
                                    // Kindle          -> "Browser"
    this["BROWSER_ENGINE"]  = "";   // Browser render engine. "Blink", "Trident", "Gecko", "WebKit"
    this["BROWSER_VERSION"] = _000; // Browser version from UserAgent String. format: "{{Major}}.{{Minor}}.{{Patch}}"

    if (ua) {
        _detectOS(this, ua);      // OS, OS_VERSION
        _detectID(this, ua);      // ID
        _detectBrowser(this, ua); // BROWSER_NAME, BROWSER_ENGINE, BROWSER_VERSION

        var id = this["ID"];

        if ( this["has"](id) ) { // cataloged?
            _setDeviceSpec(this, id);
            this["DETECTED"] = true;
        } else if (alt) {
            // unknown device -> set alternate device id
            var altID = _getAlternateDeviceID(this);

            if ( altID && this["has"](altID) ) {
                _setDeviceSpec(this, altID);
                this["ID"] = altID;
                this["DETECTED"] = false;
                this["ID_CANDIDATE"] = [id];
            }
        }
    }
}
...
Spec["prototype"]["has"]  = Spec_has;  // Spec#has(id:DeviceIDString):Boolean
Spec["prototype"]["get"]  = Spec_get;  // Spec#get(id:DeviceIDString, env:Object = {}):this|null
Spec["prototype"]["can"]  = Spec_can;  // Spec#can(feature:String):Boolean
Spec["prototype"]["dump"] = Spec_dump; // Spec#dump():Object

// --- implements ------------------------------------------
function _setDeviceSpec(that, id) {
  ...
  //that["ID"]                  = id;
    that["SOC"]                 = socid;
    that["BRAND"]               = _num2str(BRANDS, data[0]);
    that["FEATURE"]             = data[11];
  //that["LANGUAGE"]
  //that["DETECTED"]
  //that["ID_CANDIDATE"]
  //that["OS"]
...
function Spec_get(id,    // @arg DeviceIDString
                  env) { // @arg Object = {}
                         // @ret this|null
    return this["has"](id) ? _setDeviceSpec(new Spec(env), id)
                           : null;
}
...
function _getAlternateDeviceID(that) {
    var ver = parseFloat(that["OS_VERSION"]);
...
function _detectID(that, ua) {
    var id = "";

    switch (that["OS"]) {
    case "iOS":             id = _getiOSDeviceID(that, ua); break;
    case "Game":            id = _getGameDeviceID(that, ua); break;
    case "Android":         id = _getAndroidDeviceID(that, ua); break;
//  case "Fire OS":         id = _getFireOSDeviceID(that, ua); break;
    case "Windows Phone":   id = _getWindowsPhoneDeviceID(that, ua);
    }
    that["ID"] = id;
}

function _getGameDeviceID(that, ua) {
...
function _getAndroidDeviceID(that, ua) {
    // Examples:
    //      Mozilla/5.0 (Linux; U; Android 4.0.3; en-us; KFTT               Build/IML74K)      AppleWebKit/535.19 (KHTML, like Gecko) Silk/3.4 Mobile Safari/535.19 Silk-Accelerated=true
    ...
    case "Nexus 7": // -> "Nexus 7 (2013)"
        id = that["DISPLAY_DPR"] >= 2 ? "Nexus 7 (2013)" // Nexus 7 (2013)
                                      : "Nexus 7";       // Nexus 7 (2012)
    }
    return id;
}

function _getiOSDeviceID(that, ua) {
    // Examples:
    //
    ...
    that["ID_CANDIDATE"] = candidate;
    return id;
}

function _getWindowsPhoneDeviceID(that, ua) {
    // Examples:
...
function _detectBrowser(that, ua) {
    ...
    that["BROWSER_NAME"]    = browserName;
    that["BROWSER_ENGINE"]  = browserEngine;
    that["BROWSER_VERSION"] = browserVersion;
}
...
Spec["catalog"] = {
    "device": {
        ...
        //                   BRAND     SOC        OS_VER  SHORT,LONG  PPI  DPR  RAM TOUCH  INCH  FEATURE
        // --- iOS ---
...
//{@androidjp
        ...
        "F-05F":            [FUJITSU,  MSM8974AB, 442,442, 1080,1920, 480,  3, 2048,  5,   5.0,  FE_G3LN], // ARROWS NX
