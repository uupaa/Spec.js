...
var BRAND_NAMES = "APPLE=2,GOOGLE=3,AMAZON=4,NOKIA=5,SONY=6," +
                  "SHARP=7,SAMSUNG=8,LG=9,HTC=10,";
...
// --- class / interfaces ----------------------------------
function Spec(env) { // @arg Object = {} - { dpr, touch, width, height, language, userAgent }
                     // @env.alt Boolean = false - use alternate device id.
                     // @env.dpr Number = 1.0 - devicePixelRatio
                     // @env.touch Integer = 0
                     // @env.width Integer = 0
                     // @env.height Integer = 0
                     // @env.language String = "en" - content language
                     // @env.userAgent String = ""
//{@dev
    $valid($type(env, "Object|omit"), Spec, "env");
    $valid($keys(env, "alt|dpr|touch|width|height|language|userAgent"), Spec, "env");
//}@dev
    ...
    var dpr    = env["dpr"]       || global["devicePixelRatio"] || 1.0;
    var touch  = env["touch"]     || nav["maxTouchPoints"] || 0;
    var width  = env["width"]     || screen["width"]  || 0;
    var height = env["height"]    || screen["height"] || 0;
    var lang   = (env["language"] || nav["language"]  || "en").split("-", 1)[0]; // "en-us" -> "en"
    var ua     = env["userAgent"] || nav["userAgent"] || "";
    ...
    this["BRAND_NAME"] = "";        // Device brand name or device maker name (UPPER_CASE STRING). eg: "OTHER", "APPLE", "GOOGLE", "AMAZON"...
    this["FEATURE_BITS"] = 0;       // A bit pattern that indicates the features and functionality of the device.
    this["LANGUAGE"] = lang;        // Browser/Content language. "en", "ja", ...
    this["DETECTED"] = false;       // Device is described in the catalog.
    this["CANDIDATE"] = [];         // Candidate the device ID lists. If cannot be device id determined. ["iPhone 5", "iPhone 5c", "iPhone 5s"]
    // --- OS ---
    this["OS"] = os;                // The OS name. eg: "Android", "iOS", "WPhone", "Windows", "Mac", "Firefox", "Fire", "Game"
    this["OS_VERSION"] = osVer;     // Detected Current OS version from User Agent string. format "{{Major}},{{Minor}},{{Patch}}"
                                    // Convert to a floating point number are `parseFloat(spec["OS_VERSION"])`
    this["OS_RELEASE_VERSION"] = "0.0.0"; // The OS version of the device at the release date. format "{{Major}},{{Minor}},{{Patch}}"
    this["OS_HIGHEST_VERSION"] = "0.0.0"; // The OS version of the end of support. format "{{Major}},{{Minor}},{{Patch}}"
    // --- MEMORY / GPU / CPU ---
    this["RAM"] = 0.0;              // RAM MBs.
    ...
    if (ua) {
        ...
        this["CANDIDATE"] = _getCandidateID(deviceID);

        if ( this["has"](deviceID) ) { // wellknown device?
            ...
            if ( altDeviceID && this["has"](altDeviceID) ) {
                _setDeviceSpec(this, altDeviceID);
                this["DEVICE_ID"] = altDeviceID;
                this["DETECTED"] = false;
                this["CANDIDATE"] = [deviceID];
            }
        }
    }
...
Spec["prototype"]["has"]  = Spec_has;  // Spec#has(deviceID:String):Boolean
Spec["prototype"]["get"]  = Spec_get;  // Spec#get(deviceID:String, env:Object = {}):this|null
Spec["prototype"]["can"]  = Spec_can;  // Spec#can(feature:String):Boolean
Spec["prototype"]["dump"] = Spec_dump; // Spec#dump():Object
...
function _setDeviceSpec(that, deviceID) {
  ...
  //that["DEVICE_ID"]       = deviceID;
    that["SOC_ID"]          = socID;
    that["BRAND_NAME"]      = _num2str(BRAND_NAMES, data[0]);
    that["FEATURE_BITS"]    = data[11];
  //that["LANGUAGE"]
  //that["DETECTED"]
  //that["CANDIDATE"]
  //that["OS"]
  //that["OS_VERSION"]
    ...
    that["OS_HIGHEST_VERSION"] = _toVersionString(data[3]);
    that["RAM"]             = data[8];
...
function Spec_has(deviceID) { // @arg String
                              // @ret Boolean
                              // @desc device in the device catalog.
    return deviceID && deviceID in Spec["catalog"]["device"];
}

function Spec_get(deviceID, // @arg String
                  env) {    // @arg Object = {}
                            // @ret this|null
                            // @desc get device static information.
    return this["has"](deviceID) ? _setDeviceSpec(new Spec(env), deviceID)
                                 : null;
}

function Spec_can(feature) { // @arg String - "3G" or "GPS" or "LTE" or "NFC"
                             // @ret Boolean
                             // @desc can feature function.
//{@dev
    $valid($type(feature, "String"),         Spec_can, "feature");
    $valid($some(feature, "3G|GPS|LTE|NFC"), Spec_can, "feature");
//}@dev

    var value = this["FEATURE_BITS"];
...
function _overwriteVagueDeviceID(that, deviceID) {
    ...
    case "Vague_iPhone":
        ...
        if (!retina) {
            ...
            deviceID = "iPhone 5";  // CANDIDATE = ["iPhone 5", "iPhone 5c", "iPhone 5s"]
        }
        break;
    case "Vague_iPad":
        deviceID = "iPad 3";        // CANDIDATE = ["iPad 3", "iPad 4", "iPad Air", "iPad mini Retina"]
        if (!retina) {
            deviceID = "iPad 2";    // CANDIDATE = ["iPad2", "iPad mini"]
        }
        break;
    case "Vague_iPod":
...
function _getCandidateID(deviceID) {
    switch (deviceID) {
