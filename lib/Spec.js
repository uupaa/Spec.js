...
function Spec() {
    ...
    this._WEBGL_CONTEXT     = WEBGL_CONTEXT;
...
Spec["prototype"] = Object.create(Spec, {
    ...
    "WEBGL_CONTEXT":        { "get":   function()  { return this._WEBGL_CONTEXT;    } },
...
function _detectWebGL() {
    ...
    if (node) {
        for (var i = 0, iz = WEBGL_CONTEXTS.length; i < iz; ++i) {
            ...
            if (gl) {
                WEBGL_CONTEXT = token;
                GPU_FINGERPRINT = gl.getParameter(gl.VERSION);
                // - iPad mini 2 (iOS 8.1.3)  "WebGL 1.0 (OpenGL ES 2.0 Apple A7 GPU - 50.6.11)
                // - iPhone 4S   (iOS 8.1.3)  "WebGL 1.0 (OpenGL ES 2.0 IMGSGX543-113.3)
                // - iPhone 5    (iOS 8.1.3)  "WebGL 1.0 (OpenGL ES 2.0 IMGSGX543-113.3)
                // - iPhone 5    (iOS 8.3 b2) "WebGL 1.0 (OpenGL ES 2.0 IMGSGX543-115.4)
...
//      this._WEBGL_CONTEXT     = this._WEBGL_CONTEXT || "";
...
function _getiOSDeviceID(ua, osVer) {
  ...
  //var SGX535 = /535/.test(GPU_FINGERPRINT); // iPhone 3GS, iPhone 4
    var SGX543 = /543/.test(GPU_FINGERPRINT); // iPhone 4s/5/5c, iPad 2/3, iPad mini
    var SGX554 = /554/.test(GPU_FINGERPRINT); // iPad 4
    var A7     = /A7/.test(GPU_FINGERPRINT);  // iPhone 5s, iPad mini 2/3, iPad Air
    var A8     = /A8/.test(GPU_FINGERPRINT);  // iPhone 6/6+, iPad Air 2
