...
var GLES_VERSION     = 0.0; // WRITE_ONCE
var WEBGL_VERSION    = 0.0; // WRITE_ONCE
var MAX_TEXTURE_SIZE = 0;   // WRITE_ONCE
...
function Spec() { // @desc http://git.io/Spec
    ...
    this._FEATURE  = { DEVICE: 0x0, SOC: 0x0, GPU: 0x0 }; // A bit pattern that indicates the features.
    this._LIMIT    = { DEVICE: 0x0, SOC: 0x0, GPU: 0x0 }; // A bit pattern that indicates the limits.
    ...
    this._GLES_VERSION      = GLES_VERSION;     // Open GL ES Version. 0.0, 1.1, 2.0, 3.0, 3.1, 3.2
    this._WEBGL_VERSION     = WEBGL_VERSION;    // WebGL Version. 0.0, 1.0, 2.0
    this._MAX_TEXTURE_SIZE  = MAX_TEXTURE_SIZE; // GL_MAX_TEXTURE_SIZE (0, 1024 - 16384)
    // --- OS ---
...
function _collectDeviceInfo(deviceID,  // "Nexus 5"
    ...
    if (devInfo) {
        ...
        this._LOW_END = {
            "DEVICE":   !!(this._LIMIT.DEVICE & SpecCatalog["LIMIT"]["LOW_END"]),
            "GPU":      !!(this._LIMIT.GPU    & SpecCatalog["LIMIT"]["LOW_END"]),
        ...
        this._GLES_VERSION      = this._GLES_VERSION || gpuInfo[1] || 3.0;
        this._WEBGL_VERSION     = this._WEBGL_VERSION || 0.0;
        this._MAX_TEXTURE_SIZE  = this._MAX_TEXTURE_SIZE || ((gpuInfo[2] || 4) * 1024); // default 4096
        this._MAX_TOUCH_POINTS  = 0;

        if (_isNodeOrNodeWebKit || _runOnBrowser) {
            if (!this._WEBGL_VERSION) {
                _detectWebGL.call(this);
            }
            this._MAX_TOUCH_POINTS = nav["maxTouchPoints"] ||
                                     _getMaxTouchPoints(this._LIMIT.DEVICE,
                                                        SpecCatalog["LIMIT"]);
        }
    }
    function _detectWebGL() {
        ...
        for (var i = 0, iz = WEBGL_CONTEXTS.length; i < iz; ++i) {
            ...
            if (gl) {
                this._WEBGL_VERSION    = /2/.test(token) ? 2.0 : 1.0;
                this._MAX_TEXTURE_SIZE = gl.getParameter(gl.MAX_TEXTURE_SIZE);
                this._GLES_VERSION     = parseFloat( gl.getParameter(gl.VERSION).
                                                     split("ES ")[1].split(" ")[0] );
                // WRITE_ONCE
                GLES_VERSION     = this._GLES_VERSION;
                WEBGL_VERSION    = this._WEBGL_VERSION;
                MAX_TEXTURE_SIZE = this._MAX_TEXTURE_SIZE;
                break;
            }
        }
        gl   = null;
