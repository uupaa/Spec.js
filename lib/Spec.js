...
// --- class / interfaces ----------------------------------
function Spec(ua,        // @arg UserAgentObject|Object - WebModule.UserAgent object
...
Spec["prototype"] = Object.create(Spec, {
    ...
    "OPEN_GLES":        { "get":   function()  { return this._OPEN_GLES;        } }, // OpenGLES version.
    "OPEN_CL":          { "get":   function()  { return this._OPEN_CL;          } }, // OpenCL version.
...
function _initSP(ua, device, options) {
    ...
    if (record) {
        ...
        this._OPEN_GLES         = _getOpenGLESVersion(gpu[2]);
        this._OPEN_CL           = _getOpenCLVersion(gpu[2]);
        this._VULKAN            = _getVulkanVersion(gpu[2]);
...
function _getOpenCLVersion(tags) { // @arg UINT32
    if (tags & Catalog["GPU_TAGS"]["CL22"]) { return 2.2; }
    if (tags & Catalog["GPU_TAGS"]["CL21"]) { return 2.1; }
    if (tags & Catalog["GPU_TAGS"]["CL20"]) { return 2.0; }
    if (tags & Catalog["GPU_TAGS"]["CL12"]) { return 1.2; }
    if (tags & Catalog["GPU_TAGS"]["CL11"]) { return 1.1; }
    if (tags & Catalog["GPU_TAGS"]["CL10"]) { return 1.0; }
    return 0.0;
}

function _getVulkanVersion(tags) { // @arg UINT32
    if (tags & Catalog["GPU_TAGS"]["VULKAN10"]) { return 1.0; }
    return 0.0;
}
