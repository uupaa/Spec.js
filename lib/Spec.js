...
// --- define ----------------------------------------------
// --- interface -------------------------------------------
function Spec(param) { // @arg Object(= null): { ua, emu }
                       //   param.ua - UserAgentString(= navigator.userAgent):
                       //   param.emu - Object(= null): emulate spec values. { screen, devicePixelRatio, ... }
                       // @ret SpecObject: { DEVICE, OS, CPU, GPU, INPUT,
                       //                    MEMORY, DISPLAY, NETWORK, BROWSER }
                       // @help: Spec
                       // @desc: Create SpecObject.
//{@assert
    _if(!_type(param, "Object/omit", "ua,emu"), "Spec(param)");
//}@assert

    param = param || {};
    var userAgent         = param["ua"]  || (global["navigator"] || {})["userAgent"] || "";
    var emulateDeviceInfo = param["emu"] || {};

//{@assert
    _if(!_type(userAgent,         "String"), "Spec(param.ua)");
    _if(!_type(emulateDeviceInfo, "Object"), "Spec(param.emu)");
//}@assert
