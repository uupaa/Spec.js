...
var SOC_IDS = "A4=2,A5=3,A5X=4,A6=5,A6X=6,A7=7," +
       "QSD8250=20,QSD8650=21,APQ8055=22,APQ8060=23,APQ8064=24," +
       "APQ8064T=25,APQ8064AB=26,APQ8074=27,MSM7227=40,MSM7230=41," +
       "MSM8225=50,MSM8226=51,MSM8227=52,MSM8230=53,MSM8255=54," +
       "MSM8255T=55,MSM8260=56,MSM8260A=57," +
...
//}@dev
    ...
    this._DEVICE_RAM = 0.0;           // RAM MBs.
    this._DEVICE_BRAND = "";          // Device brand name or device maker name (UPPER_CASE STRING). eg: "OTHER", "APPLE", "GOOGLE", "AMAZON"...
    this._DEVICE_FEATURE = 0;         // A bit pattern that indicates the features and functionality of the device.
    this._DEVICE_DETECTED = false;    // Device is described in the catalog.
    this._DEVICE_CANDIDATE = [];      // Candidate the device ID lists. If cannot be device id determined. ["iPhone 5", "iPhone 5c", "iPhone 5s"]
    this._SOC_ID = "";                // System On Chip ID (UPPER_CASE STRING). eg: "MSM8974", "A5X"
    // --- OS ---
    this._OS = os;                    // OS name. eg: "Android", "iOS", "WPhone", "Windows", "Mac", "Firefox", "Fire", "Game"
    this._OS_VERSION = osVer;         // Detected OS version from UserAgent string. format "{{Major}},{{Minor}},{{Patch}}"
                                      // Convert to a floating point number are `parseFloat(spec["OS_VERSION"])`
    this._OS_RELASE_VERSION = "0.0.0";// OS release version. format "{{Major}},{{Minor}},{{Patch}}"
    this._OS_FINAL_VERSION = "0.0.0"; // OS final version.   format "{{Major}},{{Minor}},{{Patch}}"
    // --- GPU ---
    this._GPU_ID = "";                // GPU ID. eg: "330"
    this._GPU_TYPE = "";              // GPU type (UPPER CASE STRING). eg: "ADRENO", "TEGRA", "POWER_VR", "MALI", "IMMERSION", ...
    // --- CPU ---
    this._CPU_TYPE = "";              // CPU type. "ARM", "ARM64", "ATOM"
    this._CPU_CLOCK = 0.0;            // CPU clock, GHz.
    this._CPU_CORES = 0;              // CPU cores. 1(single core), 2(dual core), 4(quad core)
    // --- DISPLAY ---
    this._DISPLAY_DPR = dpr;          // Display Device Pixel Ratio. window.devicePixelRatio value
    this._DISPLAY_PPI = 0;            // Display Pixel Per Inch.
    this._DISPLAY_INCH = 0.0;         // Display inches.
    this._DISPLAY_LONG = long_;       // Display long edge, pixels.
    this._DISPLAY_SHORT = short_;     // Display short edge, pixels.
    this._DISPLAY_TOUCH = touch;      // Display multi touch fingers. device catalog value or navigator.maxTouchPoints value
                                      // see https://code.google.com/p/android/issues/detail?id=11909
    // --- BROWSER ---
    this._WEB_VIEW = false;           // Is WebView flag. This value set by user.
    this._USER_AGENT = ua;            // navigator.userAgent value or Spec({ userAgent: ... }) value.
                                      // Some functions refer to this value, the results will change.
    this._BROWSER_NAME = browserName; // Browser name. "Chrome", "Firefox", "Browser", "IE", "Safari", "WebKit"
                                      ...
                                      // Fire Phone      -> "Browser"
                                      // Kindle          -> "Browser"
    this._BROWSER_ENGINE = browserEngine;   // Browser render engine. "Blink", "Trident", "Gecko", "WebKit"
    this._BROWSER_VERSION = browserVersion; // Browser version from UserAgent String. format: "{{Major}}.{{Minor}}.{{Patch}}"
    this._CONTENT_LANGUAGE = lang;    // Browser/Content language. "en", "ja", ...
    ...
    if (ua) {
        ...
        if ( this["hasDeviceID"](deviceID) ) { // wellknown device?
            ...
            if ( altDeviceID && this["hasDeviceID"](altDeviceID) ) {
                ...
                this._DEVICE_ID = altDeviceID;
                this._DEVICE_DETECTED = false;
                this._DEVICE_CANDIDATE = [deviceID];
            }
        }
    }
    ...
    function _getAlternateDeviceID(os, osVer) {
        ...
        case "Android": return parseFloat(osVer) >= 4.3 ? "Nexus 5" : "XT1058";
...
Spec["prototype"] = {
    ...
    "prefix":               Spec_prefix,                                    // Spec#prefix(parent:Object, property:String):String
    // --- property accessor ---
    "getDeviceID":          function() { return this._DEVICE_ID; },         // Spec#getDeviceID():String
    "getDeviceRAM":         function() { return this._DEVICE_RAM; },        // Spec#getDeviceRAM():Number
    "getDeviceBrand":       function() { return this._DEVICE_BRAND; },      // Spec#getDeviceBrand():String
    "getDeviceFeature":     function() { return this._DEVICE_FEATURE; },    // Spec#getDeviceFeature():Integer
    "isDeviceDetected":     function() { return this._DEVICE_DETECTED; },   // Spec#isDeviceDetected():Boolean
    "getDeviceCandidate":   function() { return this._DEVICE_CANDIDATE; },  // Spec#getDeviceCandidate():StringArray
    "getSoCID":             function() { return this._SOC_ID; },            // Spec#getSoCID():String
    "getOS":                function() { return this._OS; },                // Spec#getOS():String
    "getOSVersion":         function() { return this._OS_VERSION; },        // Spec#getOSVersion():String
    "getOSReleaseVersion":  function() { return this._OS_RELASE_VERSION; }, // Spec#getOSReleaseVersion():String
    "getOSFinalVersion":    function() { return this._OS_FINAL_VERSION; },  // Spec#getOSFinalVersion():String
    "getGPUID":             function() { return this._GPU_ID; },            // Spec#getGPUID():String
    "getGPUType":           function() { return this._GPU_TYPE; },          // Spec#getGPUType():String
    "getCPUType":           function() { return this._CPU_TYPE; },          // Spec#getCPUType():String
    "getCPUClock":          function() { return this._CPU_CLOCK; },         // Spec#getCPUClock():Number
    "getCPUCores":          function() { return this._CPU_CORES; },         // Spec#getCPUCores():Integer
    "getDisplayDPR":        function() { return this._DISPLAY_DPR; },       // Spec#getDisplayDPR():Number
    "getDisplayPPI":        function() { return this._DISPLAY_PPI; },       // Spec#getDisplayPPI():Integer
    "getDisplayInch":       function() { return this._DISPLAY_INCH; },      // Spec#getDisplayInch():Number
    "getDisplayLong":       function() { return this._DISPLAY_LONG; },      // Spec#getDisplayLong():Integer
    "getDisplayShort":      function() { return this._DISPLAY_SHORT; },     // Spec#getDisplayShort():Integer
    "getDisplayTouch":      function() { return this._DISPLAY_TOUCH; },     // Spec#getDisplayTouch():Integer
    "isWebView":            function() { return this._WEB_VIEW; },          // Spec#isWebView():Boolean
    "getUserAgent":         function() { return this._USER_AGENT; },        // Spec#getUserAgent():String
    "getBrowserName":       function() { return this._BROWSER_NAME; },      // Spec#getBrowserName():String
    "getBrowserEngine":     function() { return this._BROWSER_ENGINE; },    // Spec#getBrowserEngine():String
    "getBrowserVersion":    function() { return this._BROWSER_VERSION; },   // Spec#getBrowserVersion():String
    "getContentLanguage":   function() { return this._CONTENT_LANGUAGE; },  // Spec#getContentLanguage():String
    // --- convenient ---
...
//}@dev
    ...
    return this._DEVICE_BRAND.toLowerCase() === name.toLowerCase();
}
...
//}@dev
    ...
    return this._OS.toLowerCase() === name.toLowerCase();
}
...
//}@dev
    ...
    return (this._GPU_TYPE + this._GPU_ID).toLowerCase() === name.replace(/ /g, "").toLowerCase();
}
...
//}@dev
    ...
    return this._BROWSER_NAME.toLowerCase() === name.toLowerCase();
}
...
//}@dev
    ...
    return this._BROWSER_ENGINE.toLowerCase() === name.toLowerCase();
}
...
function _setDeviceSpec(that, deviceID) {
  ...
  //that._DEVICE_ID         = deviceID;
    that._DEVICE_RAM        = data[8];
    that._DEVICE_BRAND      = _num2str(DEVICE_BRAND, data[0]);
    that._DEVICE_FEATURE    = data[11];
  //that._DEVICE_DETECTED
  //that._DEVICE_CANDIDATE
    that._SOC_ID            = socID;
  //that._OS
  //that._OS_VERSION
    that._OS_RELEASE_VERSION= _toVersionString(data[2]);
    that._OS_FINAL_VERSION  = _toVersionString(data[3]);
    that._GPU_ID            = socdata[4];
    that._GPU_TYPE          = _num2str(GPU_TYPES, socdata[3]);
    that._CPU_TYPE          = _num2str(CPU_TYPES, socdata[0]);
    that._CPU_CLOCK         = socdata[1];
    that._CPU_CORES         = socdata[2];
    that._DISPLAY_DPR       = data[7] || that["DISPLAY_DPR"];
    that._DISPLAY_PPI       = data[6];
    that._DISPLAY_INCH      = data[10];
    that._DISPLAY_LONG      = Math.max(data[4], data[5]) || that["DISPLAY_LONG"];
    that._DISPLAY_SHORT     = Math.min(data[4], data[5]) || that["DISPLAY_SHORT"];
    that._DISPLAY_TOUCH     = data[9] || that["DISPLAY_TOUCH"];
  //that._WEB_VIEW
  //that._USER_AGENT
  //that._BROWSER_NAME
  //that._BROWSER_ENGINE
  //that._BROWSER_VERSION
  //that._CONTENT_LANGUAGE
...
//}@dev
    ...
    var value = this._DEVICE_FEATURE;
...
//}@dev
    ...
    this._WEB_VIEW = value;
    return this;
}

function Spec_prefix(parent,     // @arg Object - parent Object
                     property) { // @arg String - target property
                                 // @ret String - property or vendor + Property
                                 // @desc solve vendor prefix
//{@dev
    $valid(!$type(parent, "null|undefined"), Spec_prefix, "parent");
...
function _overwriteVagueDeviceID(that, deviceID) {
    var osVer    = parseFloat(that._OS_VERSION);
    var retina   = that._DISPLAY_DPR >= 2;
    var longEdge = that._DISPLAY_LONG; // iPhone 4S: 480, iPhone 5: 568
...
Spec["catalog"] = {
    ...
    //  "Xperia Tablet Z2":
        ...
        "FXC5A":            [OTHER,    MT6582,    442,442,  540,960,    0,  0,  512,  4,     5,  FE_G3__], // JENESIS AEON geanee
...
//{@androidjp
        ...
        "SO-05F":           [SONY,     MSM8974AB, 442,442, 1200,1920, 320,  2, 3072, 10,  10.1,  FE_G3LN], // Xperia Z2 Tablet
        ...
        "F-05F":            [OTHER,    MSM8974AB, 442,442, 1080,1920, 480,  3, 2048,  5,   5.0,  FE_G3LN], // FUJITSU ARROWS NX
        ...
        "SC-01F":           [SAMSUNG,  MSM8974AA, 430,442, 1080,1920, 480,  3, 2048,  5,   5.7,  FE_G3LN], // GALAXY Note 3, S Browser
        "SC-02F":           [SAMSUNG,  MSM8974AA, 430,442, 1080,1920, 480,  3, 2048,  5,     5,  FE_G3LN], // GALAXY J SC-02F, S Browser
        ...
        "F-01F":            [OTHER,    MSM8974AA, 422,422, 1080,1776, 480,  3, 2048,  5,     5,  FE_G3LN], // FUJITSU ARROWS NX F-01F
        "F-02F":            [OTHER,    MSM8974AA, 422,422, 1504,2560, 320,  2, 2048,  5,  10.1,  FE_G3LN], // FUJITSU ARROWS Tab F-02F
        "F-03F":            [OTHER,    MSM8974AA, 422,422,  720,1184, 320,  2, 2048,  5,   4.7,  FE_G3LN], // FUJITSU Disney Mobile on docomo F-03F
        "F-04F":            [OTHER,    APQ8064T,  422,422,  540,888,  240,1.5, 2048,  5,   4.3,  FE_G3__], // FUJITSU FE_NO_STORE // (no Google Play)
        // 2013 summer
        ...
        "SC-04E":           [SAMSUNG,  APQ8064T,  422,442, 1080,1920, 480,  3, 2048,  5,     5,  FE_G3LN], // Galaxy S4, S Browser
        ...
        "F-06E":            [OTHER,    APQ8064T,  422,422, 1080,1776, 480,  3, 2048,  5,   5.2,  FE_G3LN], // FUJITSU
        "F-07E":            [OTHER,    APQ8064T,  422,422,  720,1184, 320,  2, 2048,  5,   4.7,  FE_G3LN], // FUJITSU
        "F-08E":            [OTHER,    APQ8064T,  422,422,  540,867,  240,1.5, 2048,  5,   4.3,  FE_G3L_], // FUJITSU
        "F-09E":            [OTHER,    APQ8064T,  422,422,  540,888,  240,1.5, 2048,  5,   4.3,  FE_G3L_], // FUJITSU
        // 2012 Q3
        ...
        "F-02E":            [OTHER,    AP37,      412,412, 1080,1920, 480,  0, 2048,  5,     5,  FE_G3LN], // FUJITSU
        "F-03E":            [OTHER,    MSM8960,   404,412,  540,960,  240,  0, 1024,  5,     4,  FE_G3LN], // FUJITSU
        "F-04E":            [OTHER,    AP33,      404,422,  720,1280, 320,  0, 2048,  5,   4.7,  FE_G3LN], // FUJITSU
        "F-05E":            [OTHER,    AP37,      404,412, 1200,1920, 240,  0, 2048,  5,  10.1,  FE_G3LN], // FUJITSU
        ...
        "F-09D":            [OTHER,    MSM8255,   403,403,  480,800,  240,  0, 1024,  2,   3.7,  FE_G3__], // FUJITSU
        "F-10D":            [OTHER,    AP33,      403,422,  720,1280, 323,  2, 1024,  5,   4.6,  FE_G3L_], // FUJITSU ARROWS X
        "F-11D":            [OTHER,    MSM8255,   403,422,  480,800,  240,  0, 1024,  5,   3.7,  FE_G3__], // FUJITSU
        "F-12D":            [OTHER,    MSM8255,   403,403,  480,800,  235,  0, 1024,  5,   4.0,  FE_G3__], // FUJITSU
        "T-02D":            [OTHER,    MSM8960,   404,412,  540,960,  257,  0, 1024,  5,   4.3,  FE_G3L_], // FUJITSU
        // 2011 Q3
        ...
        "F-01D":            [OTHER,    OMAP4430,  320,403,  800,1280, 160,  0, 1024,  5,  10.1,  FE_G3L_], // FUJITSU
        "F-03D":            [OTHER,    MSM8255,   235,235,  480,800,  240,  0,  512,  2,   3.7,  FE_G3__], // FUJITSU
        "F-05D":            [OTHER,    OMAP4430,  235,403,  720,1280, 342,  0, 1024,  2,   4.3,  FE_G3L_], // FUJITSU
        "F-07D":            [OTHER,    MSM8255,   235,235,  480,800,  235,  0,  512,  5,     4,  FE_G3__], // FUJITSU
        "F-08D":            [OTHER,    OMAP4430,  235,403,  720,1280, 342,  0, 1024,  2,   4.3,  FE_G3__], // FUJITSU
        "T-01D":            [OTHER,    OMAP4430,  235,403,  720,1280, 320,  0, 1024,  2,   4.3,  FE_G3__], // FUJITSU
        // 2011 Q1
        ...
        "F-12C":            [OTHER,    MSM8255,   230,230,  480,800,    0,  0,  512,  2,   3.7,  FE_G3__], // FUJITSU
        ...
        // 2013 winter
        "FJT21":            [OTHER,    MSM8974AA, 422,422, 1600,2560, 300,  0, 2048, 10,  10.1,  FE_G3LN], // FUJITSU
        ...
        "FJL22":            [OTHER,    MSM8974AA, 422,422, 1080,1920, 444,  0, 2048, 10,     5,  FE_G3LN], // FUJITSU
        ...
        "FJL21":            [OTHER,    MSM8960,   404,404,  720,1280, 342,  2, 1024, 10,   4.3,  FE_G3L_], // FUJITSU ARROWS ef
        ...
        // 2012 summer
        "ISW13F":           [OTHER,    AP33,      403,403,  720,1280, 322,  0, 1024,  3,   4.6,  FE_G3__], // FUJITSU ARROWS Z ISW13F
        ...
        "IS12F":            [OTHER,    MSM8655,   235,235,  480,800,  235,  0,  512,  4,     4,  FE_G3__], // FUJITSU ARROWS ES
        ...
        "ISW11F":           [OTHER,    OMAP4430,  235,403,  720,1280, 342,  0, 1024,  3,   4.3,  FE_G3__], // FUJITSU ARROWS Z
        ...
        "IS11T":            [OTHER,    MSM8655,   234,234,  480,854,  243,  0,  512,  3,     4,  FE_G3__], // FUJITSU REGZA Phone
        ...
        "IS04":             [OTHER,    QSD8650,   210,222,  480,854,  290,  0,  512,  2,   4.0,  FE_G3__], // FUJITSU
        ...
        "301F":             [OTHER,    MSM8974AA, 422,422, 1080,1920,   0,  0, 2048,  2,     5,  FE_G3LN], // FUJITSU
    ...
    //  "EM01L":            [GOOGLE,   MSM8974AA, 440,440, 1080,1920, 445,  3, 2048,  5,     5,  FE_G3LN], // E-Mobile Nexus 5 EM01L
        "101F":             [OTHER,    MSM8960,   404,412,  540,960,    0,  0, 1024,  2,   4.3,  FE_G3LN], // FUJITSU
        ...
        "EM01F":            [OTHER,    APQ8064,   412,412,  720,1280,   0,  0, 2048,  5,   4.7,  FE_G3LN], // FUJITSU ARROWS S EM01F
        ...
        "202F":             [OTHER,    APQ8064T,  422,422, 1080,1920,   0,  0, 2048,  2,     5,  FE_G3__], // FUJITSU
        ...
        "201F":             [OTHER,    APQ8064,   412,412,  720,1280,   0,  0, 2048,  2,   4.7,  FE_G3_N], // FUJITSU
...
//{@windowsphone
    ...
    //  "IS12T":            [OTHER,    MSM8655,   750,750,  480,800,    0,  0,  512,  4,     0,  FE_G3__], // FUJITSU
    ...
    //  "SPH-I800":         [SAMSUNG,  MSM8930,   800,800,  720,1280, 308,  0, 1024,  4,     0,  FE_G3LN], // ATIV S Neo, SoC_ID MSM8930AA -> MSM8930
...
//}@firefoxos
    ...
    //  "Exynos5422": [ARM,  2.1,  4,     MALI,     "T628MP6"       ],
        ...
        "MT6582":     [ARM,  1.3,  4,     MALI,     "400"           ], // FXC5A
