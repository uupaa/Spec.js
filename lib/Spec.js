...
//}@dev
    ...
    spec["OS_TYPE"] = _detectOSType(spec, userAgent);
    spec["OS_VERSION"] = _detectOSVersion(spec, userAgent);
    spec["ID"] = _detectID(spec, userAgent);
...
function _detectOSType(spec, ua) {
    var type = "";
...
function _detectOSVersion(spec, ua) {
    var type = spec["OS_TYPE"];
    var ver = "0.0.0";

    switch (type) {
    case "iOS":           ver = _getVersion(/OS /, ua); break;
    case "Android":
    case "Mac OS X":      ver = _getVersion(type, ua); break;
    case "Windows":       ver = _getVersion(/Windows NT/, ua); break;
    case "Windows Phone": ver = _getVersion(/Windows Phone (?:OS )?/, ua);
    }
    return ver;
}
...
//  case "Fire OS":         id = _getFireOSDeviceID(ua); break;
    ...
    case "Windows Phone":   id = _getWindowsPhoneDeviceID(ua, spec); break;
    case "Mac OS X":        break;
    case "Windows":         break;
